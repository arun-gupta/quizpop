...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: session, error: sessionError } = await supabase
      .from('game_sessions')
      .select('id, quiz_id, join_code, game_state, current_question_index, question_started_at, section_intro_at, state_changed_at, correct_answer_id, started_at, completed_at, created_at, is_paused, paused_at')
